...
function openSearch() { overlay.hidden = false; input.focus(); }
function closeSearch() { overlay.hidden = true; input.value = ''; results.innerHTML = '<p>输入关键词开始搜索</p>'; }
...
input.addEventListener('input', () => {
  ...
  results.innerHTML = matches.length ? matches.map((post) => `<a class="result-item" href="${post.querySelector('h2 a').getAttribute('href')}"><strong>${post.querySelector('h2').textContent}</strong><small>${post.querySelector('.post-excerpt').textContent.slice(0, 80)}…</small></a>`).join('') : '<p>没有找到匹配的文章。</p>';
});
menuToggle.addEventListener('click', () => { const open = nav.classList.toggle('open'); menuToggle.setAttribute('aria-expanded', String(open)); });
document.querySelectorAll('.site-nav a, .nav-search').forEach((item) => item.addEventListener('click', () => { nav.classList.remove('open'); menuToggle.setAttribute('aria-expanded', 'false'); }));
